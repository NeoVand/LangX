...
import { theme } from '$lib/transformer-explainer/constants/theme';
	import { gradientMap } from '$lib/transformer-explainer/constants/gradient';
	import {
		ATTENTION_HEAD_1,
		ATTENTION_HEAD_BACK,
		ATTENTION_OUT,
		...
		MLP,
		TRANSFORMER_BLOCKS
	} from '$lib/transformer-explainer/constants/opacity';
	import { textPages } from '$lib/transformer-explainer/utils/textbookPages';
	...
	// Map an SVG's user-space onto document coordinates, so the path generators
	// (which emit getBoundingClientRect + scroll = document coords) line up with
	// the actual elements no matter where the SVG sits on the page (e.g. below
	// LangX's 60px nav + the control bar). Without this every spline is offset by
	// the SVG's own top edge and looks disconnected.
	const applyDocumentViewBox = (node) => {
		if (!node) return;
		const rect = node.getBoundingClientRect();
		if (!rect.width || !rect.height) return;
		node.setAttribute(
			'viewBox',
			`${window.scrollX + rect.left} ${window.scrollY + rect.top} ${rect.width} ${rect.height}`
		);
	...
	const drawPath = async () => {
		...
		applyDocumentViewBox(svgEl);
		applyDocumentViewBox(svgBackEl);
