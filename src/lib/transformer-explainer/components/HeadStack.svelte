<script lang="ts">
	...
	import { gsap, Power1 } from '$lib/transformer-explainer/utils/gsap';
	import { AngleLeftOutline, AngleRightOutline } from '$lib/transformer-explainer/te-ui/icons';
...
<div class="multi-head flex w-full" data-click="attention-head">
	<div class="head-title absolute bottom-2 right-3 text-right text-gray-400">
		...
		<button
			...
			data-click="attention-head-prev-btn"><AngleLeftOutline size={20}></AngleLeftOutline></button
		>
		<button
			on:click={onClickNext}
			disabled={$isOnAnimation || disablePagination}
			data-click="attention-head-next-btn"><AngleRightOutline size={20}></AngleRightOutline></button
		>
	</div>
...
<style lang="scss">
	...
	.head-card {
		/* translucent so the Sankey ribbons (which pass under the stacked head
		   cards) stay visible on the dark theme, while the stacked-sheet effect
		   still reads as a faint grouping. */
		background-color: color-mix(in oklch, var(--color-bg-elev-2) 55%, transparent);
