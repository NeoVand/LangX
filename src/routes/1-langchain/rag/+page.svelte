<script lang="ts">
	...
	let q1Hits = $state<{ doc: Document; score: number }[]>([]);

	let q2 = $state('How do I resume a graph after a crash?');
	let q2Run = $state(false);
	let q2Hits = $state<{ doc: Document; score: number }[]>([]);
	...
	async function ensureStore() {
		...
		try {
			...
			await s.addDocuments([
				...
				new Document({ pageContent: sourceC, metadata: { source: 'Deep Agents', chapter: 3 } })
			]);
			store = s;
	...
	async function runQ1() {
		...
		try {
			...
			if (!s) return;
			q1Hits = await s.similaritySearch(q1, 3);
		} finally {
			q1Run = false;
		}
	}
	...
	async function runQ2() {
		...
		try {
			const s = await ensureStore();
			if (!s) return;
			q2Hits = await s.similaritySearch(q2, 3);
		} finally {
			q2Run = false;
		}
	}
...
<Lesson title="RAG" eyebrow="Phase 1 · Lesson 05">
	{#snippet intro()}
		<p>
			Retrieval-augmented generation: instead of training a model on your data, embed your data
			into vectors at write time, find the closest matches at read time, and stuff them into the
			prompt. Everything runs in your browser — embeddings included.
		</p>
	{/snippet}
	...
	{#snippet narrative()}
		<Slide title="The RAG sketch">
			<ul>
				<li>
					<strong>Embed</strong> each document chunk into a fixed-length vector with a small
					encoder model (we use <code>Xenova/all-MiniLM-L6-v2</code>, 384-dim, ~25 MB).
				</li>
				<li>
					<strong>Index</strong> those vectors. For tiny corpora this can be a plain array; in
					production you'd swap in pgvector, Pinecone, or Chroma.
				</li>
				<li>
					<strong>Search</strong> by embedding the query and returning the nearest neighbours
		...
		<Slide title="In your browser, on WebGPU">
			<CodeBlock code={code} caption="Indexing and searching three documents." />
			<p>
				The first run downloads the embedding model (small, cacheable). After that, embedding
				is fast — try the two demos on the right.
			</p>
		</Slide>

		<Slide title="Why no model is needed for the demo">
			<p>
				The retrieval half of RAG is what we're inspecting here — the model is just a final
				step that consumes whatever context retrieval returns. The two demos below do the
				retrieval honestly with real cosine similarity over real embeddings.
			</p>
		</Slide>

		<Slide title="Caveats">
			<p>
				In a real system: chunk before you embed, deduplicate, store metadata for citations,
				watch for context-window inflation, and consider hybrid search (BM25 + vectors). The
				machinery is small but the operational details determine whether it's actually useful.
			</p>
		</Slide>
	{/snippet}
			...
			{#if q1Hits.length}
				<ol class="hits">
					{#each q1Hits as hit, i (i)}
						<li>
							<header>
								<span class="src">{hit.doc.metadata?.source}</span>
								<span class="score">{(hit.score * 100).toFixed(1)}%</span>
							</header>
							<p>{hit.doc.pageContent}</p>
						</li>
					{/each}
				</ol>
			{/if}
